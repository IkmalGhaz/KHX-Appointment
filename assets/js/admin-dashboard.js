...
// --- Add this to the bottom of assets/js/admin-dashboard.js ---
...
if (diagBtn) {
    diagBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const testSnap = await getDocs(collection(db, "Users")); 
            const latency = Date.now() - startFetch;
            reports.push(`✅ Firestore: Connected (${latency}ms)`);

            // 2. Check ToyyibPay Gateway (via Proxy)
            // We use the same proxy logic as your booking.js
            ...
            if (gatewayCheck.ok) {
                reports.push("✅ ToyyibPay Gateway: Reachable");
            } else {
                reports.push("⚠️ ToyyibPay Gateway: Proxy Latency Detected");
            }

            // Update UI to show "Healthy"
            ...
            statusText.innerText = "Issues Detected";
            alert("Diagnostics found issues. Check console for details.");
        } finally {
